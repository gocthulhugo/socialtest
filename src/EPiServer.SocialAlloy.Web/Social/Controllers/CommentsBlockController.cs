...
namespace EPiServer.SocialAlloy.Web.Social.Controllers
{
    ...
    public class CommentsBlockController : SocialBlockController<CommentsBlock>
    {
        ...
        private void AddCommentActivity(PageComment comment)
        {
            try
            {
                var commentActivity = new PageCommentActivity { Body = comment.Body, IsImage = comment.IsImage };

                this.activityRepository.Add(comment.AuthorId, comment.PageTarget, commentActivity);
            }
            catch (SocialRepositoryException ex)
            {
                AddMessage(MessageKey, new MessageViewModel(ex.Message, ErrorMessage));
        ...
        private PageComment AdaptCommentFormViewModelToSocialComment(CommentFormViewModel formViewModel)
        {
            ...
            {
                userUri = userRepository.CreateAnonymousUri("");
            }
            else
            {
                userUri = userRepository.CreateAuthenticatedUri(User.Identity.Name);
            }

            return new PageComment
            {
                PageTarget = this.pageRepository.GetPageId(formViewModel.CurrentPageLink),
                Body = formViewModel.Body,
                //AuthorId = this.userRepository.GetUserId(this.User),
                AuthorId = formViewModel.CustomerId.ToString(),
                PhotoId = formViewModel.PhotoId,
                ProductId = formViewModel.ProductId,
                IsImage = formViewModel.IsImage,
                AuthorUsername = "Name From Customer Lookup",
                Created = DateTime.Now.ToUniversalTime(),
                IsSubComment = formViewModel.IsSubComment,
                CommentId = null,
                Parent = formViewModel.IsSubComment ? formViewModel.Parent : this.pageRepository.GetPageId(formViewModel.CurrentPageLink)
            };
        }
        ...
        private Reference CreateReferenceFor(string contentIdentifier)
        {
            return Reference.Create(contentIdentifier);
        }
    }
}
